...
import * as dataText from './response.json';
import { Options } from '@angular-slider/ngx-slider';
import * as child from './child.json';
import * as allData from './allData.json';
...
export class AppComponent implements OnInit {
  ...
  searchText() {
    dataText['last'].results.forEach((type) => {
  ...
  sendDetails(d) {
    let addresses = [];
    dataText['last'].results.forEach((data) => {
  ...
  showEntity(data) {
    ...
    const personData = dataText['last'].results.filter((ele) => {
    ...
    child.records.forEach((c) => {
      allData['first'].records.forEach((dat) => {
        console.log('condition check', c)
        if (c.record_number == dat.record_number) {
          if (
  ...
  setupChart(nodeData) {
    ...
    this.svg
      ...
      .attr(
        'd',
        (d) => `
        M${d.target.y - 1},${d.target.x + 2}
        C${d.source.y + 5},${d.target.x + 1}
         ${d.source.y + 5},${d.source.x + 1}
         ${d.source.y + 5},${d.source.x + 1}
      `
      );
  }
